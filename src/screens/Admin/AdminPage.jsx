...
import AuthUserTable, {
  NUM_ENTRIES_PER_PAGE,
} from "src/components/AuthUserTable/AuthUserTable";
import AuthUserModal from "src/components/AuthUserTable/AuthUserModal";
import ErrorPage from "src/components/ErrorPage";
import authUser from "src/pages/api/authUser";
...
// Change authUsers array to 1d array
// refactor initial load
// combine useEffects, make pagination more readable
// separate dataArray and displayArray into two states/separate fetching data/sorting data
...
const AdminPage = () => {
  ...
  useEffect(() => {
    if (loaded) {
      // Set permsissions of current user
      for (const user of Object.values(authUsers)) {
  ...
  }, [authUsers, loaded, session.user.email]);
  //   const [authUsers, setAuthUsers] = React.useState([]);
  ...
  useEffect(() => {
    async function onSessionLoad() {
      await updateData();
    }

    onSessionLoad().catch((e) => {
  ...
  for (let i = 1; i < Math.ceil(authUsers.length / NUM_ENTRIES_PER_PAGE); i++) {
                      ...
                      <Stack direction="row" wrap="wrap">
                        ...
                        {paginationNumbers.map((num) => (
                          <Button
                            bgColor="white"
                            fontFamily="body"
                            fontWeight={activePage == num ? "bold" : "normal"}
                            onClick={() => {
                              setActivePage(num);
                            }}
                            key={num}
                        ...
                        <Button
                          ...
                          fontSize={20}
                          onClick={() => {
                            if (activePage < paginationNumbers.length - 1) {
                              setActivePage(activePage + 1);
                            }
                          }}
                        >
